...
class IntegratedApp:
    ...
    def _on_canvas_station_click(self, event):
        """Maneja clicks en las estaciones del canvas para simular estados."""
        c = self.canvas_cinta
        ...
        for est, info in self.cinta_stations_info.items():
            ...
            if abs(event.x - x) <= 20 and abs(event.y - y) <= 20:
                ...
                current_state = self.station_states.get(est, False)
                self._set_cinta_station(est, not current_state)
                self._append_cinta_log(f'🖱️ Estación {est} cambiada manualmente (clic)')
                break
    ...
    def _detect_pallet_status(self, response_text):
        ...
        if 'ok' in response or 'ack' in response:
            if self._last_deliver_station is not None:
                est = self._last_deliver_station
                pal = self._last_deliver_pallet
                self._set_cinta_station(est, True, pallet_id=pal)
                self._append_cinta_log(f'✓ Pallet {pal} detectado en estación {est}')
                self._last_deliver_station = None
                self._last_deliver_pallet = None
                return
        
        # Si recibe respuesta a comando Free → pallet salió (rojo)
        if 'free' in response or 'liberar' in response:
            if self._last_free_station is not None:
                est = self._last_free_station
                pal = self._last_free_pallet
                self._set_cinta_station(est, False, pallet_id=None)
                self._append_cinta_log(f'✗ Pallet {pal} liberado de estación {est}')
                self._last_free_station = None
                self._last_free_pallet = None
                return
        
        # También detectar por patrones específicos del PLC si es necesario
        # Ejemplo: si el PLC responde con el número de estación
        for est in [1, 2, 3]:
            if f'est {est}' in response or f'estacion {est}' in response or f'station {est}' in response:
                # Determinar si es entrada o salida según el contexto
                if self._last_deliver_station == est and self._last_deliver_pallet is not None:
                    self._set_cinta_station(est, True, pallet_id=self._last_deliver_pallet)
                    self._append_cinta_log(f'✓ Pallet detectado en estación {est}')
                    self._last_deliver_station = None
                    self._last_deliver_pallet = None
                    break
                elif self._last_free_station == est:
                    self._set_cinta_station(est, False, pallet_id=None)
