...
class Aplicacion:
    def abrir_ventana_crear_aruco(self):
        """Abre la ventana independiente para crear marcadores ArUco"""
        try:
            import subprocess
            import sys
            import os
            
            # Obtener la ruta del archivo creararuco.py
            script_dir = os.path.dirname(os.path.abspath(__file__))
            creararuco_path = os.path.join(script_dir, "creararuco.py")
            
            # Ejecutar el programa independiente
            subprocess.Popen([sys.executable, creararuco_path])
        except Exception as e:
