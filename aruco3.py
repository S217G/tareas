...
class VentanaLaser:
    ...
    def crear_interfaz_laser(self):
        ...
        ttk.Button(control_row2, text="Parar Láser", command=self.parar_laser, width=15).pack(side=tk.LEFT, padx=5)
    ...
    def parar_laser(self):
        """Para el láser inmediatamente"""
        if self.ser_laser and getattr(self.ser_laser, "is_open", False):
            try:
                send_cmd(self.ser_laser, "M5")  # Comando de parada
                send_cmd(self.ser_laser, "!")   # Parada de emergencia
                self.log_laser("🛑 Láser detenido")
            except Exception as e:
...
#        btn_ordenar_azar = ttk.Button(frame_controles, text="Ordenar al Azar", command=self.boton_ordenar_azar)
#        btn_ordenar_azar.grid(row=0, column=3, padx=5, pady=5)
